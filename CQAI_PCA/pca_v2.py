...
from utils import get_image_list, load_image, apply_pca, apply_kernel_pca
...
if image is not None:
    ...
    # Create sliders for selecting the number of PCA components and compression quality
    ...
    # Add option for Kernel PCA
    use_kernel_pca = st.sidebar.checkbox("Use Kernel PCA")
    kernel_type = st.sidebar.selectbox("Kernel Type", ["rbf", "poly", "sigmoid", "linear"])
    ...
    if use_kernel_pca:
        redI = apply_kernel_pca(red, pca_components, kernel_type)
        greenI = apply_kernel_pca(green, pca_components, kernel_type)
        blueI = apply_kernel_pca(blue, pca_components, kernel_type)
    else:
